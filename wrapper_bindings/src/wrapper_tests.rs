...
use wrapper_bindings::ZZ_pX_SetCoeff_long;
...
fn mul_and_return_coeff(p: *mut ZZ) {
    unsafe {
        init_modulus(p);
        ...
        // set f to x+1
        ZZ_pX_SetCoeff_long(f, 0, 1);
        ZZ_pX_SetCoeff_long(f, 1, 1);

        // set g to 2x+1
        ZZ_pX_SetCoeff_long(g, 0, 1);
        ZZ_pX_SetCoeff_long(g, 1, 2);
